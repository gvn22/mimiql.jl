...
gxz = @ode_def_bare begin
    ...
    dx = a*(x + 1.0) - x + c*(x*x + z)
    dz = 2.0*(-z + c*(2.0*x*z))

end a c

# solve over region in parameter space
as = [10.0^i for i=-2:1:2]
cs = [10.0^i for i=-3:1:1]
...
for i ∈ CartesianIndices((1:la,1:lc))
    ...
    while S < 100.0
        ...
        sol_xy  = solve(prob_xy,ESERK5(),dense=true,abstol=1e-8,reltol=1e-8)

        prob_xz = ODEProblem(gxz,u0_xz,tspan,p)
        sol_xz  = solve(prob_xz,ESERK5(),dense=true,abstol=1e-8,reltol=1e-8)
